...
void NCE_layer<dType>::init_loss_layer(struct neuralMT_model<precision> *model,global_params &params) {
	...
	for(int i=0; i<longest_sent; i++) {
		nodes.push_back( NCE_Node<dType>(params.LSTM_size,params.minibatch_size,params.num_negative_samples,i,params.dropout) );
	}
...
void NCE_layer<dType>::back_prop2(int index) {
	...
	negative_embedding_NCE<<<256,256,0,s_layer_info.s0>>>(d_temp_D_grad,d_D_grad,d_vocab_indicies_single,num_negative_samples,LSTM_size);
	CUDA_GET_LAST_ERROR("ERROR IN KERNEL error negative embeddings positive");

	//positive embeddings update
	positive_embedding_NCE<<<256,256,0,s_layer_info.s0>>>(nodes[index].d_h_t,d_D_grad,nodes[index].d_p_true+num_negative_samples*minibatch_size,d_vocab_indicies_single+num_negative_samples,LSTM_size,minibatch_size,d_vocab_indicies_01_single);
...
void NCE_layer<dType>::dump_weights(std::ofstream &output) {
	...
	write_matrix_GPU(d_D,LSTM_size,output_vocab_size,output);
...
void NCE_layer<dType>::load_weights(std::ifstream &input) {
	...
	read_matrix_GPU(d_D,LSTM_size,output_vocab_size,input);
...
void NCE_layer<dType>::get_distribution_GPU_decoder_wrapper() {
	std::cout << "ERROR: SHOULD NOT BE HERE IN NCE CLASS DURING DECODING\n";
	exit (EXIT_FAILURE);
}
